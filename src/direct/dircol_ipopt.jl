...
"""
function parse_ipopt_summary(file=joinpath(dirname(pathof(@__MODULE__)),"..","logs","ipopt.out"))
    props = Dict()
    obj = Vector{Float64}()
...
function write_ipopt_options()
    root_dir = joinpath(dirname(pathof(@__MODULE__)),"..")
    ...
    if !isfile(optfile)
        ...
        f = open(optfile,"w")
        println(f,"# IPOPT Options for ", @__MODULE__, ".jl\n")
