...
function QuadrotorMaze(::Type{Rot}; use_rot=true, costfun=:Quadratic,
    ...
    x0 = Dynamics.build_state(model, [0, -15, 1], I(QuatRotation), zeros(3), zeros(3))
    xf = Dynamics.build_state(model, [0, 15, 1], I(QuatRotation), zeros(3), zeros(3))
